  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Valletta Fabrication Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Valletta Fabrication Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest urgency-notice-insufficient-is-held-and-unoverridable
  (testing "declared :urgency-invoked? true with a notice period below Regulation 116's own 15-day floor -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:urgency-notice-insufficient} (-> (store/ledger db) last :basis)))
...
(deftest tax-record-unverified-is-held-and-unoverridable
  (testing "unverified Commissioner for Revenue record when required -> HARD hold"
    (let [[db actor] (fresh)
