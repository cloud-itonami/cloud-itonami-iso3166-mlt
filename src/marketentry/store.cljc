(ns marketentry.store
  "SSoT for the Republic of Malta (MLT) market-entry compliance actor,
...
(defn demo-data
  ...
  `:urgency-invoked?` / `:notice-days-given` are ground truth for the
  `urgency-notice-period-insufficient?` flagship check (Public
  Procurement Regulations, Regulation 116(3) / Regulation 116(5)
  proviso, 'urgency notice period'); `:requires-tax-record?` /
  `:tax-record-verified?` are ground truth for the conditional
  Commissioner for Revenue check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Valletta Fabrication Ltd" :portal "eprocurement.gov.mt/epps"
             ...
             :jurisdiction "MLT" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "eprocurement.gov.mt/epps"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :urgency-invoked? false :notice-days-given nil
             :requires-tax-record? true :tax-record-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Birkirkara Systems Ltd" :portal "eprocurement.gov.mt/epps"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "MLT" :status :intake}
    "eng-4" {:id "eng-4" :operator "Mosta Construction Ltd" :portal "eprocurement.gov.mt/epps"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "MLT" :status :intake}
    "eng-5" {:id "eng-5" :operator "Sliema Logistics Ltd" :portal "eprocurement.gov.mt/epps"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :urgency-invoked? false :notice-days-given nil
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               urgency-invoked? notice-days-given
  ...
  (cond-> {:engagement/id id}
    ...
    (some? urgency-invoked?)              (assoc :engagement/urgency-invoked? urgency-invoked?)
    (some? notice-days-given)             (assoc :engagement/notice-days-given notice-days-given)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/urgency-invoked? :engagement/notice-days-given
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :urgency-invoked? (boolean (:engagement/urgency-invoked? m))
     :notice-days-given (:engagement/notice-days-given m)
